        ...
        <div className="w-full min-h-screen bg-white text-black m-0 p-0">
            ...
            <div className='flex flex-wrap justify-center'>
                {bills.map((bill, index) => {
                    if (settings?.customBillLayout) {
                        return (
                            <div 
                                key={index} 
                                className="no-page-break m-3 relative bg-white shrink-0 overflow-hidden" 
                                style={{ breakInside: 'avoid', pageBreakInside: 'avoid', width: `${settings.billWidth || 400}px` }}
                            >
                                <CustomBill bill={bill} settings={settings} />
                            </div>
                        );
                    }
                    return (
